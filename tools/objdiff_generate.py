...
def main():
    ...
    for file in expected_objects:
        ...
        base_path = "build/src/" + processed_path[1].removesuffix(".s.o").removesuffix(".c.o").removesuffix(".cpp.o") + ".cpp.o"
        unit = Unit(
    ...
    with (Path(config["output"])).open("w") as json_file:
        json.dump(asdict(Config(True, False, "ninja", [], units, categories)), json_file, indent=2)
