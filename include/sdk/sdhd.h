...
#define SCESDHD_NOT_IGNORE                                                     \
  (0) /*サンプルセット情報のベロシティを無視                             \
        しない*/
#define SCESDHD_IGNORE                                                         \
  (1) /*サンプルセット情報のベロシティを無視                             \
        する*/
...
typedef struct {
  ...
  unsigned int
      pitchVelCurve; /*ピッチLFOに対するベロシティカーブタイプ*/
  int amp;           /*ベロシティによるアンプLFOの変化幅*/
  unsigned int ampCenter; /*ampによる効果が０になるベロシティ値*/
  unsigned int
      ampVelCurve; /*アンプLFOに対するベロシティカーブタイプ*/
...
int sceSdHdGetSampleParamAddrByNoteVelocity(
    void *buffer, unsigned int programNumber, unsigned int noteNumber,
    unsigned int velocity, unsigned int mode, sceHardSynthSampleParam **ptr);

int sceSdHdGetSampleParamByNoteVelocity(
    void *buffer, unsigned int programNumber, unsigned int noteNumber,
    unsigned int velocity, unsigned int mode, SceSdHdSampleParam *param);
...
int sceSdHdGetVAGInfoParamAddrByNoteVelocity(
    void *buffer, unsigned int programNumber, unsigned int noteNumber,
    unsigned int velocity, unsigned int mode, sceHardSynthVagParam **ptr);

int sceSdHdGetVAGInfoParamByNoteVelocity(
    void *buffer, unsigned int programNumber, unsigned int noteNumber,
    unsigned int velocity, unsigned int mode, SceSdHdVAGInfoParam *param);
