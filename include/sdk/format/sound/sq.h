...
typedef struct {
  ...
  unsigned int maxSeSequenceSetNumber; /* max. no. of SE sequence set no.
                                        */
  unsigned int
      tableOffset; /* offset to SE sequence set offset table	(POFST)	*/
  unsigned char
      seSequenceMasterVolume;  /* master volume  */
  char seSequenceMasterPanpot; /* master panpot (0-64-127=L-C-R; negative says
                                  reverse phase) */
  unsigned short seSequenceMasterTimeScale; /* master time scale (x1=1000,
                                               slowest=1 - fastest=65535) */
  unsigned int dmy0;                        /* reserved                        */
  unsigned int dmy1;                        /* reserved                        */
  unsigned int seSequenceSetOffsetAddr[0];  /* offset list to SE sequence set
                                               (COFST)	*/
} sceSeqSeSequenceChunk;

typedef struct {
  unsigned int maxSeSequenceNumber; /* max. no. of SE sequence no. in SE
                                       sequence set	*/
  unsigned int tableOffset; /* offset to SE sequence offset table		(POFST)
                             */
  unsigned char
      seSequenceSetVolume;  /* volume  */
  char seSequenceSetPanpot; /* panpot (0-64-127=L-C-R; negative says reverse
                               phase) */
  unsigned short seSequenceSetTimeScale; /* time scale (x1=1000; slowest=1 -
                                            fastest=65535	*/
  unsigned int dmy;                      /* reserved						*/
  unsigned int seSequenceOffsetAddr[0];  /* offset list to SE sequence
                                            (COFST) */
} sceSeqSeSequenceBlock;
...
typedef struct {
  unsigned int seSequenceDataOffset; /* offset to SE sequence Data			(POFST)
                                      */
  unsigned char
      seSequenceVolume;  /* volume  */
  char seSequencePanpot; /* panpot (0-64-127=L-C-R; negative says reverse phase)
                          */
  unsigned short seSequenceTimeScale; /* time scale (x1=1000; slowest=1 -
                                         fastest=65535	*/
  unsigned int
      seSequenceDataSize; /* data size */
  unsigned int dmy;       /* reserved       */
...
typedef struct {
  ...
  unsigned int
      seSequenceChunkAddr; /* offset address: SE Sequence Chunk	(FOFST)	*/
  unsigned int
      seSongChunkAddr; /* offset address: SE Song Chunk	(FOFST)	*/
} sceSeqHeaderChunk;
