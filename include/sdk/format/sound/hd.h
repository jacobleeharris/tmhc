...
typedef struct {
  unsigned short
      VagIndex; /*CINDEX
                   VagInfoChunkでのインデックスナンバー(NO_CINDEX:SPUノイズ) */

  unsigned char
      velRangeLow; /*U8 発音下限ベロシティ(0-127) */
  unsigned char
      velCrossFade; /*U8 クロスフェイド開始ベロシティ(0-127)		*/
  unsigned char
      velRangeHigh; /*U8 発音上限ベロシティ(0-127) */

  char velFollowPitch; /*S8 ベロシティによるピッチLFOの変化幅(注1)(0-127)
                        */
  unsigned char
      velFollowPitchCenter; /*U8 velFollowPitchの効果が0になるベロシティ(0-127)
                             */
  ...
  unsigned short samplePitchLfoFade;  /*U16 Pitch LFOフェイドタイム(単位:msec)
                                       */
  unsigned short
      sampleAmpLfoDelay; /*U16 Amp LFOディレイタイム(単位:msec) */
  unsigned short
      sampleAmpLfoFade; /*U16 Amp LFOフェイドタイム(単位:msec) */

  unsigned char
      sampleLfoAttr; /*U8 LFOに関するAttribute				*/
  unsigned char
      sampleSpuAttr; /*U8 SPUに関するAttribute				*/
...
typedef struct {
  ...
  unsigned int chunkSize; /*U32 チャンクサイズ */
  unsigned int
      maxSampleNumber; /*U32 サンプルナンバーの最大値 */
...
typedef struct {
  unsigned char
      velCurve; /*U8 ベロシティカーブタイプ(for AMP LEVEL)		*/
  unsigned char
      velLimitLow; /*U8 発音上限ベロシティ */
  unsigned char
      velLimitHigh; /*U8 発音下限ベロシティ */
  unsigned char
      nSample; /*U8 このサンプルセットに属するサンプルの個数		*/

  unsigned short
      sampleIndex[0]; /*CINDEX サンプルチャンクでのインデックスナンバー	*/
...
typedef struct {
  ...
  unsigned int chunkSize; /*U32 チャンクサイズ */
  unsigned int
      maxSampleSetNumber; /*U32 サンプルセットナンバーの最大値 */

  unsigned int
      sampleSetOffsetAddr[0]; /*COFST サンプルセットテーブルへのオフセット
                               */
...
typedef struct {
  unsigned short
      sampleSetIndex; /*CINDEX サンプルセットチャンクでのインデックスナンバー
                       */

  unsigned char
      splitRangeLow;            /*U8 発音下限ノートナンバー(0-127)			*/
  unsigned char splitCrossFade; /*U8 クロスフェイド開始ノートナンバー(0-127)
                                 */
  unsigned char
      splitRangeHigh; /*U8 発音上限ノートナンバー(0-127) */
  unsigned char
      splitNumber; /*U8 スプリットナンバー(0-127) */

  unsigned short
      splitBendRangeLow; /*U16 ベンドレンジ-(注1) */
  unsigned short
      splitBendRangeHigh; /*U16 ベンドレンジ+(注1) */

  char keyFollowPitch; /*S8 1オクターブ毎に変化するピッチLFOの変化幅(注1)
                        */
  unsigned char
      keyFollowPitchCenter; /*U8 keyFollowPitch による効果が0になるノート(0-127)
                             */
  char keyFollowAmp;        /*S8 1オクターブ毎に変化するアンプLFOの変化幅
                             */
  unsigned char keyFollowAmpCenter; /*U8 keyFollowAmp
                                       による効果が0になるノート(0-127)	*/
  char keyFollowPan; /*S8 1オクターブ毎に変化するパンポットの変化幅
                      */
  unsigned char keyFollowPanCenter; /*U8 keyFollowPan
                                       による効果が0になるノート(0-127)	*/
...
typedef struct {
  unsigned int
      splitBlockAddr; /*POFST スプリットブロックへのオフセット		*/

  unsigned char
      nSplit; /*U8 このプログラムに属する splitBlock の個数		*/
  unsigned char
      sizeSplitBlock; /*U8 ひとつの splitBlock のサイズ			*/
  ...
  unsigned char
      progLfoWave; /*U8 LFO波形(ピッチ)					*/
  unsigned char
      progLfoWave2;                 /*U8 LFO波形(アンプ)                 */
  ...
  unsigned char progLfoStartPhase2; /*U8 LFOスタート位相(アンプ)(0-255:0-359°)
                                     */
  unsigned char
      progLfoPhaseRandom; /*U8 LFOランダムスタート位相(ピッチ)(0-255:0-359°)
                           */
  unsigned char
      progLfoPhaseRandom2; /*U8 LFOランダムスタート位相(アンプ)(0-255:0-359°)
                            */
  unsigned short progLfoFreq;  /*U16 LFO周期(ピッチ)(単位:msec)  */
  unsigned short progLfoFreq2; /*U16 LFO周期(アンプ)(単位:msec) */
  short
      progLfoPitchDepth;        /*S16 ピッチモジュレーション振幅+(注1)        */
...
typedef struct {
  ...
  unsigned char seTimbreNoteVelCurve; /*U8 ノートベロシティカーブタイプ(0-5)
                                       */
  unsigned char
      seTimbreNoteVolume; /*U8 ノートボリューム(0-128)			*/
  char
      seTimbreNotePanpot; /*S8 ノートパンポット(0-64-127=左-中央-右;負は逆相) */
  char seTimbreNoteTranspose; /*S8 ノートトランスポーズ(単位ノート)
                               */
  char seTimbreNoteDetune;    /*S8 ノートディチューン(注1)			*/
  unsigned char
      seTimbreNoteGroup; /*U8 ノートグループ(0-127;0はグループなし)	*/
  unsigned char seTimbreNoteGroupLimit; /*U8
                                           ノートグループ内発音数リミット(0-48;0はリミットなし)
                                         */
  ...
  unsigned char
      seTimbreNotePitchLfoWave; /*U8  LFO 波形(ピッチ) */
  unsigned char
      seTimbreNoteAmpLfoWave;                    /*U8  LFO 波形(アンプ)                    */
  unsigned char seTimbreNotePitchLfoStartPhase;  /*U8  LFO
                                                    スタート位相(ピッチ)(0-255:0-359°)
                                                  */
  unsigned char seTimbreNoteAmpLfoStartPhase;    /*U8  LFO
                                                    スタート位相(アンプ)(0-255:0-359°)
                                                  */
  unsigned char seTimbreNotePitchLfoRandomPhase; /*U8  LFO
                                                    ランダムスタート位相(ピッチ)(0-255:0-359°)
                                                  */
  unsigned char seTimbreNoteAmpLfoRandomPhase;   /*U8  LFO
                                                    ランダムスタート位相(アンプ)(0-255:0-359°)
                                                  */
  ...
  short
      seTimbreNotePitchLfoDepth2; /*S16 LFO 振幅-(ピッチ)(注1) */
  char seTimbreNoteAmpLfoDepth;   /*S8  LFO 振幅+(アンプ)   */
  char
      seTimbreNoteAmpLfoDepth2; /*S8  LFO 振幅-(アンプ) */

  unsigned short
      seTimbreNotePitchLfoDelay; /*U16 LFO ディレイタイム(単位:msec)(ピッチ)
                                  */
  unsigned short
      seTimbreNoteAmpLfoDelay; /*U16 LFO ディレイタイム(単位:msec)(アンプ)
                                */
  unsigned short
      seTimbreNotePitchLfoFade; /*U16 LFO フェイドタイム(単位:msec)(ピッチ)
                                 */
  unsigned short
      seTimbreNoteAmpLfoFade;      /*U16 LFO フェイドタイム(単位:msec)(アンプ)
                                    */
  char seTimbreNoteVelFollowPitch; /*S8
                                      ベロシティによるピッチLFOの変化幅(-127..0..127)(注1)
                                    */
  char seTimbreNoteVelFollowAmp;   /*S8
                                      ベロシティによるアンプLFOの変化幅(-127..0..127)
                                    */
  unsigned char seTimbreNoteVelFollowPitchCenter; /*U8
                                                     seTimbreNoteVelFollowPitchの効果が0になるベロシティ(1-127)
                                                   */
  unsigned char seTimbreNoteVelFollowAmpCenter;   /*U8
                                                     seTimbreNoteVelFollowAmpの効果が0になるベロシティ(1-127)
                                                   */
  unsigned char seTimbreNoteVelFollowPitchCurve;  /*U8
                                                     seTimbreNoteVelFollowPitchの効果量を定めるためのベロシティカーブ(0-9)
                                                   */
  unsigned char seTimbreNoteVelFollowAmpCurve;    /*U8
                                                     seTimbreNoteVelFollowAmpの効果量を定めるためのベロシティカーブ(0-9)
                                                   */
  unsigned char dmy;                              /*U8 予約領域					*/
...
typedef struct {
  unsigned int
      seTimbreNoteBlockAddr; /*POFST 効果音ティンバーノートの先頭へのオフセット
                              */
  unsigned char
      seTimbreNoteBlockSize; /*U8 ひとつの効果音ドラムノートブロックのサイズ
                              */
  ...
  sceHardSynthSeTimbreNoteBlock
      seTimbreNoteBlock[0]; /* 効果音ティンバーノートブロック(個数分)	*/
...
typedef struct {
  ...
  unsigned int chunkSize; /*U32 チャンクサイズ		*/
  unsigned int
      maxSeTimbreSetNumber; /*U32 効果音ティンバーセットナンバーの最大値 */

  unsigned int
      seTimbreSetOffsetAddr[0]; /*COFST 効果音ティンバーセットへのオフセット */
